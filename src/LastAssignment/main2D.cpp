...
void paintGL(GLuint programId, GLFWwindow* window, CA2D& ca) {
    // 背景色と深度値のクリア
        ...
        for(int j = 0; j < LENGTH; j++) {
            if(field[i][j]) {
                glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_INT, 
                        (void*)(((i * LENGTH) + j) * sizeof(GLuint) * 36));
            }
        }
    }
...
int main(int argc, char **argv) {
    ...
    std::vector<int> birth_condition{3};
    std::vector<int> alive_condition{2, 3};
    CA2D ca2d = CA2D(LENGTH, birth_condition, alive_condition, 0.05, false, false);

    while (glfwWindowShouldClose(window) == GLFW_FALSE) {
        paintGL(programId, window, ca2d);
        ca2d.progressField();
